...
c = 3e8;
lambda = c/f_c;         %lambda
f_lowIF1 = 5e4;  %Low intermediate frequency
f_lowIF = 5e4;
...
IF_t = cos(pi/4+2*pi*f_lowIF1*t+(h_t+r_t)*4*pi/lambda);
...
I_t = lowpass(I_t, 10 , Fs);
Q_t = lowpass(Q_t, 10 , Fs);
...
Diff_I = diff(I_t);
Diff_Q = diff(Q_t);
Diff_MDACM1 = lambda/(4*pi)*( I_t(1:Ns-1).* Diff_Q - Diff_I.*Q_t(1:Ns-1));
Diff_MDACM = lambda/(4*pi)*(I_t(1:Ns-1).*Q_t(2:Ns)-I_t(2:Ns).*Q_t(1:Ns-1));    %atan /rad    atand /degree
...
figure(5)
subplot(2,1,1)
plot(t(2:Ns), Diff_MDACM(1:Ns-1));
xlabel("Time(s)");
ylabel("Difference of x_MDACM");
...
figure(8)
subplot(2,1,1)
plot(f_axis(1:1000), I_t_fft(1:1000));
xlabel('Fre(Hz)')
subplot(2,1,2)
plot(f_axis(1:1000), Q_t_fft(1:1000));
xlabel('Fre(Hz')
